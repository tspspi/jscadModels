...
*/
    ...
    include('/physics/semcathodes/LEOLeicaCambridgeAEI.jscad');
		...
		function optics() {
			...
			let cathode = union(
				...
				difference(
					cylinder({ d : 9, h : 7, center: true }).translate([0,0,-5.5+3.5]),
					cylinder({ d : 7.5, h : 7, center: true }).translate([0,0,-5.5+3.5-0.5]),
					cylinder({ d : 0.05, h : 7, center: true }).translate([0,0,-5.5+3.5])
				)
			);

			let plate2 = difference(
				segmentPlateTemplate(),
				cylinder( { d : 1, h : 0.5, center : true }).translate([0,0,0.5/2])
			);

			let l1 = difference(
				segmentPlateTemplate(),
				cylinder( { d : 16, h : 0.5, center: true }).translate([0,0,0.5/2])
			);
		...
		function opticsclamp() {
				...
				body = difference(body, cylinder({ d : 35, h : ch, center : true }).translate([0,0,ch/2]));
    ...
    function main() {
        let cathode = new window.jscad.tspi.physics.semcathodes.LEOLeicaCambridgeAEI({}, {});
				...
				let stepper = new window.jscad.tspi.mechanics.stepper28BYJ_48({},{});
				...
				let stepperMount = difference(
					...
					union(
						cylinder({ d : 27, h : stepperMount_h, center : true }).translate([0,0,-15-stepperMount_h/2]),
						...
						cylinder( { d : 4.5, h : 66*2, center : true }).rotateX(90).translate([ 35/2, 0, -7.5-gearwheel.pitchDiameter-8]),
						cylinder( { d : 4.5, h : 66*2, center : true }).rotateY(90).translate([0,  35/2, -7.5-gearwheel.pitchDiameter-8]),
						cylinder( { d : 4.5, h : 66*2, center : true }).rotateX(90).translate([-35/2, 0, -7.5-gearwheel.pitchDiameter-8]),
						cylinder( { d : 4.5, h : 66*2, center : true }).rotateY(90).translate([0, -35/2, -7.5-gearwheel.pitchDiameter-8])

					)
				);
			stepperMount = difference(stepperMount, stepper.getModel().rotateZ(180).rotateX(-90).translate([0,33,-7.5-gearwheel.pitchDiameter]));
			stepperMount = difference(stepperMount, stepper.getModel().rotateZ(180).rotateX(-90).translate([0,33,-7.5-gearwheel.pitchDiameter]).rotateZ(90));
			...
			let nuts = union(
				nutm4.getModel().rotateZ(30).rotateX(-90).translate([ 35/2, 33-7, -7.5-gearwheel.pitchDiameter-8]),
				nutm4.getModel().rotateZ(30).rotateX(-90).translate([ -35/2, 33-7, -7.5-gearwheel.pitchDiameter-8]),
				nutm4.getModel().rotateZ(30).rotateX(-90).translate([ 35/2, 33-7, -7.5-gearwheel.pitchDiameter-8]).rotateZ(90),
				nutm4.getModel().rotateZ(30).rotateX(-90).translate([ -35/2, 33-7, -7.5-gearwheel.pitchDiameter-8]).rotateZ(90)
			).setColor([0.95, 0.95, 0.95]);


			let rodsassembly = union(
				cylinder( { d : 6 , h : 170, center : true } ).translate([-15,-15,0]),
				cylinder( { d : 6 , h : 170, center : true } ).translate([-15, 15,0]),
				cylinder( { d : 6 , h : 170, center : true } ).translate([ 15,-15,0]),
				cylinder( { d : 6 , h : 170, center : true } ).translate([ 15, 15,0])
			).setColor([0,1,0]);
        ...
        return union(
						...
						gearwheel.getModel().rotateX(90).translate([0,42,-7.5]),
						gearwheel.getModel().rotateX(90).translate([0,42,-7.5]).rotateZ(90),
						...
						stepperMount.setColor([0.3176,0.1922,0.2157]),
